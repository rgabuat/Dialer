@extends('admin.layouts.app', ['heading' => 'New Campaign'])
        ...
        <form method="POST" action="{{ route('admin.campaigns.store') }}" class="space-y-5">
            ...
            <div class="bg-surface border border-surface rounded-xl p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <select name="type" required
                            ...
                            @foreach ($types as $t)
                                <option value="{{ $t }}" {{ old('type', 'OUTBOUND') === $t ? 'selected' : '' }}>
                                    {{ $t }}</option>
                            @endforeach
                        </select>
                        @error('type')
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="dial_mode" required
                            ...
                            @foreach ($dialModes as $m)
                                <option value="{{ $m }}"
                                    {{ old('dial_mode', 'MANUAL') === $m ? 'selected' : '' }}>{{ $m }}</option>
                            @endforeach
                        </select>
                        @error('dial_mode')
                            <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="caller_id" value="{{ old('caller_id') }}" placeholder="+1xxxxxxxxxx"
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="number" name="dial_level" value="{{ old('dial_level', 1) }}" min="0"
                            step="0.01"
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="number" name="acw_seconds" value="{{ old('acw_seconds', 0) }}" min="0"
                            class="w-full bg-surface-2 border border-surface rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-zinc-500 transition">
                    </div>
                    <div class="space-y-1">
                        <label class="block text-xs font-medium text-fg-muted">Hopper Level</label>
                        <input type="number" name="hopper_level" value="{{ old('hopper_level', 100) }}" min="0"
                            class="w-full bg-surface-2 border border-surface rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-zinc-500 transition">
                    </div>
                    <div class="space-y-1">
                        <label class="block text-xs font-medium text-fg-muted">Max Calls</label>
                        <input type="number" name="max_calls" value="{{ old('max_calls', 0) }}" min="0"
                            class="w-full bg-surface-2 border border-surface rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-zinc-500 transition">
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="recording_enabled" value="1"
                            {{ old('recording_enabled') ? 'checked' : '' }} class="sr-only peer">
                        <div
                            class="w-9 h-5 bg-zinc-700 peer-focus:outline-none rounded-full peer peer-checked:bg-indigo-600 transition after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4">
                        </div>
                    </label>
                ...
                <div class="space-y-1">
                    ...
                    <select name="recording_channels"
                        ...
                        @foreach ($recChannels as $ch)
                            <option value="{{ $ch }}"
                                {{ old('recording_channels', 'both') === $ch ? 'selected' : '' }}>
                                {{ ucfirst($ch) }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="bg-surface border border-surface rounded-xl p-5 space-y-2">
                ...
                <textarea name="script" rows="6"
                    ...
                    placeholder="Agent script for this campaign…">{{ old('script') }}</textarea>
            </div>
