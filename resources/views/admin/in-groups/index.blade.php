@extends('admin.layouts.app', ['heading' => 'Inbound Groups'])

@section('content')
    @livewire('admin.in-group-modal')

    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('ingroup-saved', () => setTimeout(() => window.location.reload(), 500));
        });
    </script>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-fg">Inbound Groups</h2>
                <p class="text-sm text-fg-muted mt-0.5">Manage inbound call queues and routing rules.</p>
            </div>
            <button type="button" onclick="Livewire.dispatch('open-ingroup-create')"
        ...
        <div class="bg-surface border border-surface rounded-xl overflow-hidden">
            ...
            <div class="flex items-center justify-between gap-3 px-5 py-3 border-b border-surface">
                <form method="GET" action="{{ route('admin.in-groups.index') }}" class="flex items-center gap-2">
                ...
                <table class="min-w-full text-sm text-fg">
                    <thead class="bg-surface">
                        <tr class="border-b border-surface text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                            ...
                            <th class="px-5 py-3 text-left">Agents</th>
                    ...
                    <tbody>
                        @forelse ($groups as $group)
                            ...
                            <tr class="border-b border-surface hover:bg-hover transition">
                                ...
                                <td class="px-5 py-3">
                                    <span class="px-2 py-0.5 rounded text-xs font-medium bg-surface-2 text-fg-muted">
                                        {{ $routingLabels[$group->agent_routing] ?? $group->agent_routing }}
                                    </span>
                                </td>
                                <td class="px-5 py-3 text-xs text-fg-muted">{{ $group->queue_priority }}</td>
                                <td class="px-5 py-3 text-xs text-fg-muted">{{ $group->users_count }}</td>
                                ...
                                <td class="px-5 py-3 text-xs text-fg-muted">
                                    {{ $group->hours_json ? 'Business hours' : '24/7' }}
                                </td>
                                <td class="px-5 py-3">
                                    @if ($group->is_active)
                                ...
                                <td class="px-5 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button type="button"
                                            onclick="Livewire.dispatch('open-ingroup-edit', { id: {{ $group->id }} })"
                                        ...
                                        <form method="POST" action="{{ route('admin.in-groups.destroy', $group) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-5 py-16 text-center text-fg-muted">
                                    <p class="text-sm font-medium">No inbound groups yet</p>
                                    <p class="text-xs mt-1">Click <strong>New Inbound Group</strong> to create the first
                                        one.</p>
                                </td>
                            </tr>
                        @endforelse
