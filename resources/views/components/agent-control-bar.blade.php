...
@persist('agent-phone-bar')
    <div x-data="agentPhone()" @make-call.window="makeCall($event.detail)"
        ...
        if (!deviceReady && ['phones','outbound'].includes(($event.detail.statusName || '').toLowerCase())) enableCalling();
    "
        class="flex items-center gap-2">

        {{-- ── CALL CONTROLS pill ──────────────────────────────────── --}}
        <div class="flex items-center gap-1 bg-surface px-1.5 py-1 border border-surface rounded-lg">
        ...
        {{-- ── SEPARATOR ───────────────────────────────────────────── --}}
        <div class="bg-surface-2 w-px h-5"></div>
...
@persist('incoming-call-modal')
    <div x-data="{ show: false, callerNumber: '' }" @incoming-call.window="show = true; callerNumber = $event.detail" x-show="show" x-cloak
        ...
        <div class="z-10 relative bg-surface shadow-2xl border border-surface rounded-2xl w-full max-w-xs overflow-hidden text-center animate-modal-in"
            @click.stop>

            {{-- Animated ring --}}
            <div class="relative flex justify-center items-center pt-8 pb-4">
                {{-- outer ripples --}}
                <span class="absolute bg-green-500/10 rounded-full w-24 h-24 animate-ping"
                    style="animation-duration:1.4s"></span>
                <span class="absolute bg-green-500/15 rounded-full w-16 h-16 animate-ping"
                    style="animation-duration:1s"></span>
                {{-- icon circle --}}
                <span
                    class="inline-flex relative justify-center items-center bg-green-500/20 border border-green-500/30 rounded-full w-14 h-14">
            ...
            {{-- Info --}}
            <div class="px-6 pb-2">
                <p class="mb-1 font-medium text-fg-muted text-xs uppercase tracking-widest">Incoming Call</p>
                <p class="font-semibold text-fg text-lg truncate" x-text="callerNumber || 'Unknown caller'"></p>
                {{-- ringing dots --}}
                <p class="flex justify-center items-center gap-0.5 mt-1 text-fg-muted text-xs">
                    Ringing
                    <span class="inline-flex gap-0.5 ml-1">
                        <span class="bg-fg-muted rounded-full w-1 h-1 animate-bounce" style="animation-delay:0ms"></span>
                        <span class="bg-fg-muted rounded-full w-1 h-1 animate-bounce"
            ...
            <div class="flex gap-3 px-6 py-5">
                <button @click="$dispatch('reject-call'); show = false"
                    class="inline-flex flex-1 justify-center items-center gap-2 bg-red-600/15 hover:bg-red-600/25 py-3 border border-red-500/20 rounded-xl font-semibold text-red-500 text-sm transition">
                ...
                <button @click="$dispatch('accept-call'); show = false"
                    class="inline-flex flex-1 justify-center items-center gap-2 bg-green-600 hover:bg-green-500 shadow-sm py-3 rounded-xl font-semibold text-white text-sm transition">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M1.5 4.5a3 3 0 0 1 3-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 0 1-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 0 0 6.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 0 1 1.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 0 1-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5Z" />
                    </svg>
