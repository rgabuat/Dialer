...
<div x-data="{
    ...
    <button type="button" @click="open = !open"
        class="inline-flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 px-3 py-1.5 border border-zinc-800 hover:border-zinc-700 focus:border-zinc-600 rounded-lg focus:outline-none text-sm whitespace-nowrap transition select-none"
        :class="open ? 'border-zinc-600 bg-zinc-800' : ''" {{ $attributes }}>
        ...
        {{-- label --}}
        <span :class="selected ? 'text-white' : 'text-zinc-500'" x-text="label ?? '{{ $placeholder }}'"></span>
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-150"
        ...
        class="absolute top-full mt-1.5 z-50 min-w-[160px] bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl shadow-black/60 py-1 origin-top-{{ $align }}"
        :class="'{{ $align }}'
        ...
        <button type="button" @click="pick('')" class="flex items-center gap-2.5 px-3 py-2 w-full text-sm transition"
            :class="selected === '' ? 'text-white bg-zinc-800' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'">
