        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <div class="max-w-2xl mx-auto px-8 py-6 space-y-5">
                <div>
                    ...
                    <p class="text-xs text-fg-muted mt-0.5">Name and description for this lead form template.</p>
                </div>
                ...
                @if (count($lead_process_steps) === 0)
                    <div class="flex-1 flex items-center justify-center">
                        <div class="text-center space-y-3">
                            <p class="text-sm text-fg-muted">No steps yet — add a step to start building.</p>
                            <button type="button" wire:click="addLeadStep"
                                ...
                                <div class="space-y-1">
                                    @foreach ($fieldLibrary as $lf)
                                        <button type="button"
                                            ...
                                            {{ $lf['label'] }}
                                            <svg class="w-3 h-3 text-fg-muted" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
                                    ...
                                    <div class="bg-surface border border-surface rounded-xl p-3 space-y-3"
                                        ...
                                        <div class="space-y-2 min-h-[60px] border border-dashed border-surface rounded-lg p-2"
                                            ...
                                            @if (empty($stepData['fields']))
                                                <p class="text-[11px] text-fg-muted text-center py-4">Drop inputs here</p>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                @foreach ($lead_process_steps as $si => $s)
                                    @foreach (($s['fields'] ?? []) as $fi => $field)
                                        <div x-show="sel.step==={{ $si }}&&sel.field==={{ $fi }}"
                                            wire:key="lt-props-{{ $si }}-{{ $field['uid'] ?? $fi }}" class="space-y-2.5">
                                            <div>
                                            ...
                                            <div>
                                                <label class="text-[11px] text-fg-muted">Placeholder</label>
                                                <input type="text" wire:model.blur="lead_process_steps.{{ $si }}.fields.{{ $fi }}.placeholder"
                                                    class="w-full bg-surface-2 border border-surface rounded px-2 py-1 text-xs text-fg focus:outline-none focus:ring-1 focus:ring-indigo-500 mt-0.5">
                                            </div>
                                            @if (($field['type'] ?? '') === 'select')
                                                <div>
                                                    <label class="text-[11px] text-fg-muted">Options (one per line)</label>
                                                    <textarea wire:model.blur="lead_process_steps.{{ $si }}.fields.{{ $fi }}.options_text"
                                                        rows="4" class="w-full bg-surface-2 border border-surface rounded px-2 py-1 text-xs text-fg focus:outline-none focus:ring-1 focus:ring-indigo-500 mt-0.5 resize-none"></textarea>
                                                </div>
                                            @endif
                                            <label class="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" wire:model="lead_process_steps.{{ $si }}.fields.{{ $fi }}.required"
