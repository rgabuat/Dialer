<div class="flex flex-col gap-0 h-full">
    ...
    <div class="flex items-center px-6 border-surface border-b shrink-0">
        @foreach (['overview' => 'Overview', 'conversations' => 'Conversations', 'leads' => 'Leads', 'agents' => 'Agents', 'campaigns' => 'Call Volume'] as $key => $label)
    ...
    <div class="flex-1 overflow-y-auto">
        ...
        @if ($tab === 'overview')
            @php
                $ovDailyLabels = json_encode(
                    array_map(fn($d) => \Carbon\Carbon::parse($d)->format('j M'), array_keys($dailyVolume)),
                );
                ...
                $chBg = json_encode(
                    $byChannel
                        ...
                        ->map(
                            fn($c) => match ($c) {
                                'voice' => 'rgba(59,130,246,0.8)',
                                'sms' => 'rgba(168,85,247,0.8)',
                                'email' => 'rgba(249,115,22,0.8)',
                                default => 'rgba(20,184,166,0.8)',
                            },
                        )
                        ->values()
                        ->all(),
                ...
                <div class="space-y-6 p-6">
                    ...
                    <div class="gap-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            ...
                            <p class="font-bold text-fg text-2xl leading-tight">{{ number_format($totalConversations) }}
                            </p>
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            ...
                            <div class="flex items-center gap-1.5 pt-1">
                                <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                    <div class="bg-green-400 rounded-full h-1.5" style="width: {{ $completionRate }}%">
                                    </div>
                                </div>
                                <span
                                    class="text-fg-muted/60 text-xs shrink-0">{{ number_format($completedConversations) }}</span>
                            </div>
                        </div>
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            ...
                            <p class="pt-1 text-fg-muted/60 text-xs">
                                @if ($totalLeadValue > 0)
                                    ${{ number_format($totalLeadValue) }} value
                                @else
                                    Created in period
                                @endif
                            </p>
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            ...
                            <div class="flex items-center gap-1.5 pt-1">
                                <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                    <div class="bg-fuchsia-500 rounded-full h-1.5"
                                        style="width: {{ $conversionRate }}%"></div>
                                </div>
                                <span
                                    class="text-fg-muted/60 text-xs shrink-0">{{ number_format($convertedLeads) }}</span>
                            </div>
                        </div>
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                    ...
                    <div class="bg-surface p-4 border border-surface rounded-xl" x-data="{
                        init() {
                    ...
                    @if ($totalLeads > 0)
                        <div class="gap-4 grid grid-cols-1 md:grid-cols-2">
                            ...
                            @php
                                ...
                                $psData = json_encode(
                                    array_map(
                                        fn($k) => $leadsByStage->firstWhere('pipeline_stage', $k)?->total ?? 0,
                                        array_keys($stageLabels),
                                    ),
                                );
                                $psBg = json_encode([
                                    'rgba(250,204,21,0.8)',
                    ...
                    @if ($byCampaign->isNotEmpty())
                        <div class="bg-surface border border-surface rounded-xl overflow-hidden">
                            <div class="flex justify-between items-center px-4 py-3 border-surface border-b">
                                ...
                                <button wire:click="$set('tab', 'campaigns')"
                                    class="text-fuchsia-400 hover:text-fuchsia-300 text-xs transition">View all
                                    →</button>
                            </div>
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-surface">
                                    @foreach ($byCampaign->take(5) as $row)
                                        <tr>
                                            <td class="px-4 py-2.5 font-medium text-fg text-xs">
                                                {{ $row->campaign?->name ?? 'Unknown' }}</td>
                                            <td class="px-4 py-2.5 text-fg text-xs text-right">
                                                {{ number_format($row->total) }}</td>
                                            <td class="px-4 py-2.5 text-xs text-right">
                                                <span
                                                    class="text-green-400">{{ number_format($row->completed) }}</span>
                                                <span
                                                    class="ml-1 text-fg-muted/60">{{ $row->total ? round(($row->completed / $row->total) * 100) : 0 }}%</span>
                                            </td>
                                            <td class="px-4 py-2.5 text-fg-muted text-xs text-right">
                                                @if ($row->avg_dur)
                                                    {{ floor($row->avg_dur / 60) }}m {{ $row->avg_dur % 60 }}s
                                                @else
                                                    —
                                                @endif
                                            </td>
        ...
        @if ($tab === 'conversations')
            @php
                ...
                $dispLabels = json_encode(
                    $byDisposition->map(fn($r) => $r->disposition?->name ?? 'Unknown')->values()->all(),
                );
                ...
                <div class="space-y-6 p-6">
                    ...
                    <div class="gap-3 grid grid-cols-2 md:grid-cols-4">
                        ...
                        <div class="bg-surface p-4 border border-surface rounded-xl">
                            ...
                            <p class="mt-1 font-bold text-fg text-2xl">{{ number_format($completedConversations) }}
                            </p>
                            <p class="mt-0.5 text-green-400 text-xs">{{ $completionRate }}% rate</p>
                        </div>
                        <div class="bg-surface p-4 border border-surface rounded-xl">
                            <p class="font-medium text-fg-muted text-xs">Abandoned</p>
                            <p class="mt-1 font-bold text-fg text-2xl">{{ number_format($abandonedConversations) }}
                            </p>
                    ...
                    @if ($byDisposition->isNotEmpty())
                        ...
                        <div class="bg-surface p-4 border border-surface rounded-xl" x-data="{
                            ...
                            <div class="flex justify-between items-center mb-3">
                                ...
                                <p class="text-fg-muted/60 text-xs">{{ number_format($byDisposition->sum('total')) }}
                                    dispositioned</p>
                            </div>
                            <div style="height: {{ max(180, $byDisposition->count() * 36) }}px"><canvas></canvas>
                            </div>
                        </div>
                    @endif
        ...
        @if ($tab === 'leads')
            @php
                ...
                $ldStageData = json_encode(
                    array_map(
                        fn($k) => $leadsByStage->firstWhere('pipeline_stage', $k)?->total ?? 0,
                        array_keys($stageLabels),
                    ),
                );
                $ldStageBg = json_encode([
                    'rgba(250,204,21,0.8)',
                    'rgba(74,222,128,0.8)',
                    'rgba(161,161,170,0.5)',
                    'rgba(161,161,170,0.5)',
                ...
                $ldStoreLbls = json_encode(
                    $leadsByStore->map(fn($r) => $r->store?->name ?? 'Unknown')->values()->all(),
                );
                $ldStoreData = json_encode($leadsByStore->pluck('total')->values()->all());

                $ldAgentLbls = json_encode(
                    $leadsByAgent->map(fn($r) => $r->creator?->name ?? 'Unknown')->values()->all(),
                );
        ...
        @if ($tab === 'agents')
            @php
                $agLbls = json_encode(
                    $byAgent->map(fn($r) => $r->assignedAgent?->name ?? 'Unassigned')->values()->all(),
                );
                    ...
                    <div class="bg-surface border border-surface rounded-xl overflow-hidden">
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-surface">
                                @forelse ($byAgent as $row)
                                    @php
                                        ...
                                        $initials =
                                            ...
                                            strtoupper(substr(strstr($name, ' ') ?: '', 1, 1));
                                    ...
                                    <tr>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                <div
                                                    class="flex justify-center items-center bg-fuchsia-500/30 rounded-full w-8 h-8 font-bold text-fuchsia-200 text-xs shrink-0">
                                                    {{ $initials ?: '?' }}</div>
                                                <span class="font-semibold text-fg text-xs">{{ $name }}</span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 font-semibold text-fg text-sm text-right">
                                            {{ number_format($row->total) }}</td>
                                        <td class="px-4 py-3 font-semibold text-green-400 text-xs text-right">
                                            {{ number_format($row->completed) }}</td>
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end items-center gap-2">
                                                <div class="bg-surface-2 rounded-full w-16 h-1.5">
                                                    <div class="bg-green-400 rounded-full h-1.5"
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            @if ($leadsCount > 0)
                                                <span
                                                    class="inline-flex items-center bg-fuchsia-500/15 px-2 py-0.5 rounded-full font-semibold text-fuchsia-300 text-xs">{{ $leadsCount }}</span>
                                            @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-10 text-fg-muted text-sm text-center">No
                                            agent data for this period</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        {{-- ═══════════════════════════════════════════════════════════════════
             CALL VOLUME TAB  (user-scoped campaigns)
        ════════════════════════════════════════════════════════════════════ --}}
        @if ($tab === 'campaigns')
            @php
                $cvAvgMins = $cvAvgDur ? floor($cvAvgDur / 60) . 'm ' . $cvAvgDur % 60 . 's' : '—';
                $cvCompRate = $cvTotalCalls ? round(($cvCompleted / $cvTotalCalls) * 100) : 0;
                $cvAbanRate = $cvTotalCalls ? round(($cvAbandoned / $cvTotalCalls) * 100) : 0;
                $cvAnswerRate = $cvTotalCalls ? round((($cvTotalCalls - $cvAbandoned) / $cvTotalCalls) * 100) : 0;

                $cvCpLbls = json_encode(
                    $callVolumeByCampaign->map(fn($r) => $r->campaign?->name ?? 'No Campaign')->values()->all(),
                );
                $cvCpInbound = json_encode($callVolumeByCampaign->pluck('inbound')->values()->all());
                $cvCpOutbound = json_encode($callVolumeByCampaign->pluck('outbound')->values()->all());
            @endphp
            @if ($callVolumeByCampaign->isEmpty())
                <div class="flex flex-col justify-center items-center py-24 text-center">
                    <div class="flex justify-center items-center bg-surface-2 mb-5 rounded-2xl w-16 h-16">
                        <x-heroicon-o-phone class="w-8 h-8 text-fg-muted/40" />
                    </div>
                    <p class="font-semibold text-fg text-sm">No call data for this period</p>
                    <p class="mt-1 text-fg-muted text-xs">No campaigns are assigned to your group or no calls were
                        made.</p>
                </div>
            @else
                <div class="space-y-6 p-6">
                    ...
                    <div class="gap-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-phone class="w-3.5 h-3.5 text-fg-muted/60" />
                                <p class="font-medium text-fg-muted text-xs">Total Calls</p>
                            </div>
                            <p class="font-bold text-fg text-2xl leading-tight">{{ number_format($cvTotalCalls) }}</p>
                            <p class="pt-1 text-fg-muted/60 text-xs">{{ $callVolumeByCampaign->count() }}
                                campaign{{ $callVolumeByCampaign->count() === 1 ? '' : 's' }}</p>
                        </div>

                        {{-- Inbound --}}
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-arrow-down-left class="w-3.5 h-3.5 text-blue-400" />
                                <p class="font-medium text-fg-muted text-xs">Inbound</p>
                            </div>
                            <p class="font-bold text-blue-400 text-2xl leading-tight">{{ number_format($cvInbound) }}
                            </p>
                            <p class="pt-1 text-fg-muted/60 text-xs">
                                {{ $cvTotalCalls ? round(($cvInbound / $cvTotalCalls) * 100) : 0 }}% of total</p>
                        </div>

                        {{-- Outbound --}}
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-arrow-up-right class="w-3.5 h-3.5 text-fuchsia-400" />
                                <p class="font-medium text-fg-muted text-xs">Outbound</p>
                            </div>
                            <p class="font-bold text-fuchsia-400 text-2xl leading-tight">
                                {{ number_format($cvOutbound) }}</p>
                            <p class="pt-1 text-fg-muted/60 text-xs">
                                {{ $cvTotalCalls ? round(($cvOutbound / $cvTotalCalls) * 100) : 0 }}% of total</p>
                        </div>

                        {{-- Answered --}}
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-check-circle class="w-3.5 h-3.5 text-green-400" />
                                <p class="font-medium text-fg-muted text-xs">Answered</p>
                            </div>
                            <p class="font-bold text-green-400 text-2xl leading-tight">
                                {{ number_format($cvCompleted) }}</p>
                            <div class="flex items-center gap-1.5 pt-1">
                                <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                    <div class="bg-green-400 rounded-full h-1.5" style="width: {{ $cvCompRate }}%">
                                    </div>
                                </div>
                                <span class="text-fg-muted/60 text-xs shrink-0">{{ $cvCompRate }}%</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-x-circle class="w-3.5 h-3.5 text-red-400" />
                                <p class="font-medium text-fg-muted text-xs">Abandoned</p>
                            </div>
                            <p class="font-bold text-red-400 text-2xl leading-tight">{{ number_format($cvAbandoned) }}
                            </p>
                            <div class="flex items-center gap-1.5 pt-1">
                                <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                    <div class="bg-red-400 rounded-full h-1.5" style="width: {{ $cvAbanRate }}%">
                                    </div>
                                </div>
                                <span class="text-fg-muted/60 text-xs shrink-0">{{ $cvAbanRate }}%</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1 bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-1.5">
                                <x-heroicon-o-clock class="w-3.5 h-3.5 text-fg-muted/60" />
                                <p class="font-medium text-fg-muted text-xs">Avg Handle Time</p>
                            </div>
                            <p class="font-bold text-fg text-2xl leading-tight">{{ $cvAvgMins }}</p>
                            <p class="pt-1 text-fg-muted/60 text-xs">Answered calls</p>
                        </div>

                    </div>

                    {{-- ── Inbound vs Outbound grouped bar chart per campaign ─────────── --}}
                    <div class="bg-surface p-4 border border-surface rounded-xl" x-data="{
                        init() {
                            const ctx = this.$el.querySelector('canvas').getContext('2d');
                            new Chart(ctx, {
                                ...
                                data: {
                                    labels: {{ $cvCpLbls }},
                                    datasets: [
                                        { label: 'Inbound', data: {{ $cvCpInbound }}, backgroundColor: 'rgba(59,130,246,0.7)', borderRadius: 4, borderWidth: 0 },
                                        { label: 'Outbound', data: {{ $cvCpOutbound }}, backgroundColor: 'rgba(168,85,247,0.7)', borderRadius: 4, borderWidth: 0 }
                                    ]
                                },
                                options: {
                                    responsive: true,
                        ...
                        <div class="flex justify-between items-center mb-3">
                            <p class="font-semibold text-fg text-sm">Inbound vs Outbound by Campaign</p>
                            <div class="flex items-center gap-3 text-xs text-fg-muted">
                                <span class="flex items-center gap-1"><span
                                        class="inline-block w-2.5 h-2.5 rounded-sm bg-blue-500/70"></span>Inbound</span>
                                <span class="flex items-center gap-1"><span
                                        class="inline-block w-2.5 h-2.5 rounded-sm bg-fuchsia-500/70"></span>Outbound</span>
                            </div>
                        </div>
                        <div style="height: {{ max(200, $callVolumeByCampaign->count() * 55) }}px"><canvas></canvas>
                        </div>
                    </div>

                    {{-- ── Per-campaign detail table ───────────────────────────────────── --}}
                    <div class="bg-surface border border-surface rounded-xl overflow-hidden">
                        <div class="flex justify-between items-center px-4 py-3 border-surface border-b">
                            <div>
                                <p class="font-semibold text-fg text-sm">Campaign Call Volume</p>
                                <p class="mt-0.5 text-fg-muted text-xs">{{ $rangeLabels[$range] ?? '' }} ·
                                    {{ $callVolumeByCampaign->count() }}
                                    campaign{{ $callVolumeByCampaign->count() === 1 ? '' : 's' }}</p>
                            </div>
                            <button wire:click="export" wire:loading.attr="disabled"
                                class="inline-flex items-center gap-1.5 bg-surface-2 hover:bg-surface-3 px-2.5 py-1 rounded-md text-xs text-fg-muted transition">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full min-w-[700px]">
                                <thead>
                                    <tr
                                        class="border-surface border-b bg-surface-2/40 font-semibold text-fg-muted text-xs uppercase tracking-wide">
                                        <th class="px-4 py-2.5 text-left">Campaign</th>
                                        <th class="px-4 py-2.5 text-right">Total</th>
                                        <th class="px-4 py-2.5 text-right text-blue-400">Inbound</th>
                                        <th class="px-4 py-2.5 text-right text-fuchsia-400">Outbound</th>
                                        <th class="px-4 py-2.5 text-right">Answered</th>
                                        <th class="px-4 py-2.5 text-right">Abandoned</th>
                                        <th class="px-4 py-2.5 text-right">Answer%</th>
                                        <th class="px-4 py-2.5 text-right">Avg Duration</th>
                                        <th class="px-4 py-2.5 text-left w-28">Share</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-surface">
                                    @forelse ($callVolumeByCampaign as $row)
                                        @php
                                            $rowCompPct = $row->total
                                                ? round(($row->completed / $row->total) * 100)
                                                : 0;
                                            $rowAbanPct = $row->total
                                                ? round((($row->abandoned ?? 0) / $row->total) * 100)
                                                : 0;
                                            $rowSharePct = round(($row->total / $cvMaxTotal) * 100);
                                        @endphp
                                        <tr class="hover:bg-surface-2/30 transition-colors">
                                            {{-- Campaign name + mode badge --}}
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-2.5">
                                                    <div
                                                        class="flex justify-center items-center bg-blue-500/15 rounded-lg w-8 h-8 shrink-0">
                                                        <x-heroicon-o-megaphone class="w-4 h-4 text-blue-400" />
                                                    </div>
                                                    <div>
                                                        <p class="font-semibold text-fg text-xs leading-tight">
                                                            {{ $row->campaign?->name ?? 'No Campaign' }}</p>
                                                        @if ($row->campaign?->dial_mode)
                                                            <span
                                                                class="text-fg-muted/60 text-[10px] capitalize">{{ $row->campaign->dial_mode }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                            {{-- Total --}}
                                            <td class="px-4 py-3 font-bold text-fg text-sm text-right">
                                                {{ number_format($row->total) }}</td>
                                            {{-- Inbound --}}
                                            <td class="px-4 py-3 text-right">
                                                <span
                                                    class="inline-flex items-center gap-1 font-semibold text-blue-400 text-xs">
                                                    <x-heroicon-o-arrow-down-left class="w-3 h-3" />
                                                    {{ number_format($row->inbound ?? 0) }}
                                                </span>
                                            </td>
                                            {{-- Outbound --}}
                                            <td class="px-4 py-3 text-right">
                                                <span
                                                    class="inline-flex items-center gap-1 font-semibold text-fuchsia-400 text-xs">
                                            ...
                                            {{-- Abandoned --}}
                                            <td class="px-4 py-3 text-right">
                                                @if (($row->abandoned ?? 0) > 0)
                                                    <span
                                                        class="font-semibold text-red-400 text-xs">{{ number_format($row->abandoned) }}</span>
                                                @else
                                    ...
                                    {{ $rowCompPct >= 70 ? 'text-green-400' : ($rowCompPct >= 40 ? 'text-yellow-400' : 'text-red-400') }}">
                                                    {{ $rowCompPct }}%
                                                </span>
                                            </td>
                                            {{-- Avg Duration --}}
                                            ...
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-2">
                                                    <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                                        <div class="bg-blue-500/60 rounded-full h-1.5"
                                                            style="width: {{ $rowSharePct }}%"></div>
                                                    </div>
                                                    <span
                                                        class="w-8 text-fg-muted/60 text-xs text-right">{{ $cvTotalCalls ? round(($row->total / $cvTotalCalls) * 100) : 0 }}%</span>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="9" class="px-4 py-10 text-fg-muted text-sm text-center">No
                                                campaign data for this period</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                {{-- Totals footer --}}
                                @if ($callVolumeByCampaign->count() > 1)
                                    <tfoot>
                                        <tr
                                            class="border-surface border-t bg-surface-2/30 font-semibold text-fg-muted text-xs">
                                            <td class="px-4 py-2.5">Totals</td>
                                            <td class="px-4 py-2.5 text-fg text-right">
                                                {{ number_format($cvTotalCalls) }}</td>
                                            <td class="px-4 py-2.5 text-blue-400 text-right">
                                                {{ number_format($cvInbound) }}</td>
                                            <td class="px-4 py-2.5 text-fuchsia-400 text-right">
                                                {{ number_format($cvOutbound) }}</td>
                                            <td class="px-4 py-2.5 text-green-400 text-right">
                                                {{ number_format($cvCompleted) }}</td>
                                            <td class="px-4 py-2.5 text-red-400 text-right">
                                                {{ number_format($cvAbandoned) }}</td>
                                            <td
                                                class="px-4 py-2.5 text-right {{ $cvCompRate >= 70 ? 'text-green-400' : ($cvCompRate >= 40 ? 'text-yellow-400' : 'text-red-400') }}">
                                                {{ $cvCompRate }}%</td>
                                            <td class="px-4 py-2.5 text-right">{{ $cvAvgMins }}</td>
                    ...
                    {{-- ── Inbound / Outbound summary cards ───────────────────────────── --}}
                    <div class="gap-4 grid grid-cols-1 md:grid-cols-2">

                        {{-- Inbound breakdown --}}
                        <div class="bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-2 mb-4">
                                <div class="flex justify-center items-center bg-blue-500/15 rounded-lg w-8 h-8">
                                    <x-heroicon-o-arrow-down-left class="w-4 h-4 text-blue-400" />
                                </div>
                                <div>
                                    <p class="font-semibold text-fg text-sm">Inbound Calls</p>
                                    <p class="text-fg-muted text-xs">Calls received across campaigns</p>
                                </div>
                            </div>
                            <div class="space-y-3">
                                @forelse ($callVolumeByCampaign->where('inbound', '>', 0)->sortByDesc('inbound') as $row)
                                    @php $pct = $cvInbound > 0 ? round(($row->inbound ?? 0) / $cvInbound * 100) : 0; @endphp
                                    <div>
                                        <div class="flex justify-between items-center mb-1">
                                            <span
                                                class="font-medium text-fg text-xs">{{ $row->campaign?->name ?? 'No Campaign' }}</span>
                                            <span
                                                class="font-bold text-blue-400 text-xs">{{ number_format($row->inbound ?? 0) }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                                <div class="bg-blue-500/70 rounded-full h-1.5"
                                                    style="width: {{ $pct }}%"></div>
                                            </div>
                                            <span
                                                class="w-8 text-fg-muted/60 text-xs text-right">{{ $pct }}%</span>
                                        </div>
                                    </div>
                                @empty
                                    <p class="text-fg-muted/60 text-xs text-center py-4">No inbound calls</p>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="bg-surface p-4 border border-surface rounded-xl">
                            <div class="flex items-center gap-2 mb-4">
                                <div class="flex justify-center items-center bg-fuchsia-500/15 rounded-lg w-8 h-8">
                                    <x-heroicon-o-arrow-up-right class="w-4 h-4 text-fuchsia-400" />
                                </div>
                                <div>
                                    <p class="font-semibold text-fg text-sm">Outbound Calls</p>
                                    <p class="text-fg-muted text-xs">Calls dialed across campaigns</p>
                                </div>
                            </div>
                            <div class="space-y-3">
                                @forelse ($callVolumeByCampaign->where('outbound', '>', 0)->sortByDesc('outbound') as $row)
                                    @php $pct = $cvOutbound > 0 ? round(($row->outbound ?? 0) / $cvOutbound * 100) : 0; @endphp
                                    <div>
                                        <div class="flex justify-between items-center mb-1">
                                            <span
                                                class="font-medium text-fg text-xs">{{ $row->campaign?->name ?? 'No Campaign' }}</span>
                                            <span
                                                class="font-bold text-fuchsia-400 text-xs">{{ number_format($row->outbound ?? 0) }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <div class="flex-1 bg-surface-2 rounded-full h-1.5">
                                                <div class="bg-fuchsia-500/70 rounded-full h-1.5"
                                                    style="width: {{ $pct }}%"></div>
                                            </div>
                                            <span
                                                class="w-8 text-fg-muted/60 text-xs text-right">{{ $pct }}%</span>
                                ...
                                @empty
                                    <p class="text-fg-muted/60 text-xs text-center py-4">No outbound calls</p>
                                @endforelse
                            </div>
                        </div>
